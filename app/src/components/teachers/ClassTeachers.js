...
class ClassTeachers extends Component {
  ...
  componentDidMount() {
    let uri = this.props.api.requests[ClassTeachersList].replace(
      "{id}",
      this.props.match.params.id
    );

    axios(uri).then(resp => resp.data).then(teachers => {
  ...
  render() {
    ...
    return (
      <Segment basic loading={isLoading}>
        <TeachersList teachers={teachers} actionRel={REMOVE_CLASS_TEACHER} />
        {teachers.actions &&
          <CustomForm
            action={SirenHelpers.getAction(teachers, ADD_CLASS_TEACHER)}
          />}
      </Segment>
    );
  }
